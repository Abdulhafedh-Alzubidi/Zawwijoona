...
import { auth } from './firebase'; // استدعاء إعدادات قاعدة البيانات
...
export default function App() {
  ...
  // Teams structure: id, name, leader, tasks, members[]
  const [teams, setTeams] = useState([
    { id: 1, name: "فريق الاستقبال", leader: "سالم", tasks: "استقبال الضيوف وتطييبهم بالبخور", members: [] },
    { id: 2, name: "فريق الضيافة", leader: "عبدالله", tasks: "الإشراف على توزيع القهوة والمديد", members: [] }
  ]);

  const [events, setEvents] = useState([
    { id: 'contract', titleKey: 'contract', title: "", date: "", location: "", notes: "" },
    { id: 'henna', titleKey: 'henna', title: "", date: "", location: "", notes: "" },
    { id: 'magyal', titleKey: 'magyal', title: "", date: "", location: "", notes: "" },
    { id: 'banquet', titleKey: 'banquet', title: "", date: "", location: "", notes: "" },
  ]);
            ...
            {events.map((eventInfo) => (
              <div key={eventInfo.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 transition hover:border-amber-200">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-lg font-bold text-gray-800">{eventInfo.titleKey ? t[eventInfo.titleKey] : eventInfo.title}</h3>
  ...
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'} className={`min-h-screen bg-[#fdfbf7] font-sans ${isRtl ? 'font-arabic' : ''}`}>
      ...
      <nav className="relative bg-white shadow-sm border-b border-gray-200 sticky top-0 z-30">
        ...
        {isProfileMenuOpen && (
          <div className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-full mt-2 w-80 rounded-3xl bg-white border border-gray-200 shadow-2xl z-40 overflow-hidden`}>
            {!isProfileEditMode ? (
              <>
              ...
              <form onSubmit={handleProfileSave} className="p-4 space-y-3">
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button type="submit" className="flex-1 px-3 py-2 text-sm font-semibold text-white rounded-2xl bg-amber-600 hover:bg-amber-700 transition">
                    {t.save}
                  </button>
                </div>
              </form>
            ...
            <form onSubmit={(e) => {
              ...
              setTeams(teams.map(team => {
                if (team.id === showTeamMemberModal) {
                  return { ...team, members: [...team.members, { id: Date.now(), name: formData.get('memberName') }] };
                }
                return team;
              }));
